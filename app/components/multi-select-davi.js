  ...
  isSelected: Ember.computed('value', 'selection', function (_, checked) {
    if (arguments.length > 1) {
      ...
      if (checked && !selected) {
        ...
        this.get('component').send('selected', this.get('value'));
      } else if (!checked && selected) {
        this.get('selection').removeObject(this.get('value'));
        this.get('component').send('deselected', this.get('value'));
      }
    }
...
export default Ember.Component.extend({
  ...
  actions: {
    selected: function(value) {
      this.sendAction("selectAction", value);

    },
    deselected: function(value) {
      this.sendAction("deselectAction", value);
    }
  }
});
